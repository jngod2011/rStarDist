function [LL,LLt,StDraw,m,V] = ffbs_kflfvg(y,u,A,B,C,D,F,G,S,m0,V0,bsInd)
...
%% Last-updated Time-stamp: <2018-05-31 18:14:55 (m1kfl00)>
%%
...
if maxDiffX < symTol
    ...
    error('SigXX1 is too asymmetric, something is wrong')
end

if maxDiffY < symTol
    SigYY1 = 0.5*(SigYY1 + SigYY1');
else
    error('SigYY1 is too asymmetric, something is wrong')
end
...
for i = 2:T
    ...
    if maxDiffX < symTol
        ...
        error('SigXX is too asymmetric, something is wrong')
    end
     
    if maxDiffY < symTol
        SigYY = 0.5*(SigYY + SigYY');
    else
        error('SigYY is too asymmetric, something is wrong')
    end
...
if bsInd == 1
    ...
    if maxDiffV < symTol
        ...
        error('VT is too asymmetric, something is wrong')
    end
    
    [VV,DD] = eig(VT);
    
    if (min(DD) < -1e-8)
        error('There is an eigenvalue in VT that is TOO negative');
    else
        VT = VV*abs(DD)*VV';
    end

    StDraw(:,T) = mvnrnd(m(:,T),VT);
    ...
    for j = (T-1):-1:1
        ...
        mu1 = m(:,j) + S1*(StDraw(:,(j+1)) - (A*m(:,j) + B*u(:,j+1))) ;
        Sig1 = V(:,:,j) - S1*A*V(:,:,j) ;
        Sig1 = Sig1/2+Sig1'/2 ;
        [VV,DD] = eig(Sig1) ;
        
        if (min(DD) < -1e-8)
            error('There is an eigenvalue in Sig1 that is TOO negative');
        else
            Sig1 = VV*abs(DD)*VV';
        end
                
        StDraw(:,j) = mvnrnd(mu1,Sig1);
